...
class PedidoService:
    def __init__(self, produtos_persistencia: ProdutosPersistencia):
        ...
        self.cupons: List[Tuple[Optional[str], Callable[[float], float]]] = [
            ...
            (
                "LUB2",
                lambda p, prod_tipo=None: p - 2 if prod_tipo == "lubrificante" else p,
            ),
        ]

    def _aplicar_cupom(
        self, preco: float, cupom: Optional[str], prod_tipo: str
    ) -> float:
        if not cupom:
            return preco
    ...
    def processar_pedido(self, p: Dict) -> float:
        ...
        if pedido.preco_bruto < 0:
            ...
            raise PrecoNegativoError(
                f"Preço bruto negativo para pedido: {pedido.preco_bruto}"
            )
